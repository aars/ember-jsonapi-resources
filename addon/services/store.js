...
/*
 * Caching version of ember-jsonapi-resource store.
 *
 * The original store is just a facade for the service of given `type`, which is just a facade
 * for the adapter. It offers only the ease of not having to get the service yourself, and no
 * other functionality.
 *
 * This store relies heavily on the ResourceOperations mixin, expecting most operations to go
 * through Resource instances (which will use respective service/adapter). It removes the
 * "redundant" createResource, createRelationship, etc, methods and provides a very simple API
 * to create, find and peek resources.
 *
